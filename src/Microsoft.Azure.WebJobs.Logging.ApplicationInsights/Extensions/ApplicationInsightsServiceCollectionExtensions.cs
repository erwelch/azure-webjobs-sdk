    ...
    internal static class ApplicationInsightsServiceCollectionExtensions
    {
        ...
        public static IServiceCollection AddApplicationInsights(this IServiceCollection services)
        {
            ...
            services.AddSingleton<ITelemetryInitializer, WebJobsTelemetryInitializer>(provider =>
            {
                ...
                return new WebJobsTelemetryInitializer($"{options.SdkVersionPrefix}: {assemblyVersion}");
            });
            ...
            services.AddSingleton<ITelemetryModule, DependencyTrackingTelemetryModule>(provider =>
            {
                ...
                dependencyCollector.EnableW3CHeadersInjection = !options.DisableW3CDistributedTracing;
                ...
                var requestCollector =
                    new RequestTrackingTelemetryModule(appIdProvider)
                    {
                        CollectionOptions = new RequestCollectionOptions
                        {
                            ...
                            EnableW3CDistributedTracing = !options.DisableW3CDistributedTracing,
            ...
            services.AddSingleton<TelemetryClient>(provider =>
            {
                ...
                ApplicationInsightsLoggerOptions options = 
                    provider.GetService<IOptions<ApplicationInsightsLoggerOptions>>().Value;

                TelemetryClient client = new TelemetryClient(configuration);

                client.Context.GetInternalContext().SdkVersion = $"{options.SdkVersionPrefix}: {assemblyVersion}";
            ...
            LoggerFilterOptions filterOptions)
        {
            if (options.InstrumentationKey != null)
            {
                ...
                if (string.IsNullOrEmpty(TelemetryConfiguration.Active.InstrumentationKey))
                {
                    ...
                    if (!options.DisableW3CDistributedTracing)
                    {
